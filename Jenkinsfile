            ...
            steps {
                withCredentials([file(credentialsId: 'kube', variable: 'KUBECONFIG')]) {
                    script {
                        ...
                        kubectl --kubeconfig $KUBECONFIG --insecure-skip-tls-verify set image deployment/train-schedule train-schedule=$DOCKER_IMAGE:$DOCKER_TAG
                        kubectl --kubeconfig $KUBECONFIG --insecure-skip-tls-verify rollout status deployment/train-schedule -n $KUBE_NAMESPACE || exit 1
                        """
                    }
                }
            }
